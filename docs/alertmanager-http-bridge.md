...
- **A delivery's own deterministic destination session decodes, but its
  ...
  result.
- **A delivery loses a `receivedAt` ordering race against a concurrent
  request for the same fingerprint.** `receivedAt` is generated by the
  receiver before a webhook enters the per-fingerprint lock
  (`FingerprintLock`), so two concurrent HTTP bodies for the same
  fingerprint can be timestamped in one order but reach the reducer in the
  other. When that happens the reducer's own `received_at_regression`
  rejection fires — but the bridge does not surface it as an ordinary `207`
  per-alert rejection, because doing so would let the losing request's
  caller believe its delivery was durably evaluated and settled when it was
  actually never applied. Instead the bridge throws `DeliveryOrderingError`
  (`errors.ts`), which the HTTP layer maps to `503`
  (`delivery_ordering_conflict`) and records as a sanitized `fail_closed`
  audit event with `errorType: "ordering"`. This is a transient condition,
  not a stuck one: it clears itself the moment the concurrent winning
  delivery has been applied, and a retry of the losing request then
  succeeds (or dedupes, if by the time it retries its own content has
  already been subsumed) — no operator intervention or state repair is
  required.
...
- **A checkpoint's delivery identity is recomputed on load, not trusted from
  disk.** `startsAt`/`endsAt`/`receivedAt` must each already be in the exact
  canonical ISO form `canonicalTimestamp` produces — a value that parses to
  the same instant but isn't byte-identical (different precision, a `+00:00`
  offset instead of `Z`, ...) is rejected rather than silently normalized,
  so nothing downstream can drift from what was actually validated.
  `deliveryId` must equal `createAlertmanagerDeliveryId({ alertStatus,
  fingerprint, startsAt, endsAt })` recomputed from the checkpoint's own
  other fields (the same exported hash `planAlertDeliveryIngestion` uses,
  not a second copy) — so a checkpoint file tampered or corrupted to change
  `startsAt`/`alertStatus`/`endsAt` while leaving a now-stale `deliveryId`
  in place fails to decode instead of being replayed as if it were still the
  delivery that hash originally described.
...
A fingerprint's checkpoint slot is also singular by design, but it is
first-write-wins, not last-write-wins: the *first* delivery that needs to
defer for a fingerprint is held durably, and a *differing* delivery
(`deliveryId` mismatch) that would also need to defer while that checkpoint
is still held is rejected with `503` (`CheckpointConflictError`, see
"Occurrence routing" above) rather than overwriting it — the bridge never
discards an already-durable, already-2xx-acknowledged checkpoint in favor of
a newer one. This is a deliberate scope bound, not a queue: only one
deferred delivery per fingerprint can ever be outstanding at a time,
mirroring `reduceAlertDelivery`'s own single-current-occurrence model — see
the reducer's contract in `docs/incident-state-v3.md`.
...
A's own state updates correctly without ever being treated as orphaned), a
`receivedAt` ordering conflict (a concurrent delivery for the same
fingerprint wins the race; the losing request gets `503`
`delivery_ordering_conflict` with the destination state left unchanged, and
a retry after the race resolves succeeds), `503` when the Gateway is
unreachable, a Gateway restart with the bridge left running, two bridge
restarts (one while the checkpoint is still held, one after it has been
replayed), and the crash window described above (including the
destination-overwrite guard).
